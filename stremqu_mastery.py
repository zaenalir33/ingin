...
def download_video_from_url(url: str, slot: int, filename_hint: str = "") -> str:
    ...
    if "drive.google.com" in parsed.netloc or "docs.google.com" in parsed.netloc:
        try:
            ...
            raise RuntimeError(
                "Library gdown belum terpasang. Tambahkan gdown di requirements.txt."
            ) from exc
        ...
        if not result or not target.exists() or target.stat().st_size == 0:
            raise RuntimeError(
                "Google Drive gagal diunduh. Pastikan file disetel 'Anyone with the link'."
            )
...
def run_ffmpeg(
    ...
    duration_seconds = (
        ...
        if playback_mode == "Durasi streaming" and duration_hours
        else None
    )
    ...
    if mode == "Video + MP3":
        ...
        if not video_paths or not audio_paths:
            log_callback(
                "ERROR: Mode Video + MP3 membutuhkan 1 video dan minimal 1 MP3."
            )
            return

        # Audio realtime agar antrean audio tidak membesar.
        if playback_mode == "Jumlah pengulangan":
            audio_playlist = make_audio_playlist(
                audio_paths,
                repeat_count
            )
            audio_loop_args = []
        ...
        log_callback(
            "Audio dibaca realtime (-re) agar antrean tidak menumpuk."
        )

        if playback_mode == "Jumlah pengulangan":
            log_callback(
                f"Playlist MP3 diputar tepat {repeat_count} kali, lalu streaming berhenti."
            )

        elif playback_mode == "Durasi streaming":
            log_callback(
                f"Streaming dibatasi {duration_hours:g} jam."
            )

        else:
            log_callback(
                "MP3: 1 → 2 → 3 → 4 → 5 → kembali ke 1, loop terus."
            )

        log_callback(
            "Video di-loop terus; audio asli video tidak digunakan."
        )

        log_callback(
            "Menjalankan FFmpeg ke YouTube..."
        )
        ...
        if not video_paths:
            log_callback(
                "ERROR: Minimal 1 video diperlukan."
            )
            return
        ...
        log_callback(
            "Playlist: Video 1 → Video 2 → Video 3 → Video 4 → Video 5"
        )

        if playback_mode == "Tanpa batas":
            log_callback(
                "Playlist video akan loop terus."
            )

        elif playback_mode == "Jumlah pengulangan":
            log_callback(
                f"Playlist 5 Video diputar tepat {repeat_count} kali, lalu streaming berhenti."
            )
    ...
    try:
        ...
        log_callback(
            f"FFmpeg berhenti dengan kode: {process.returncode}"
        )

    except FileNotFoundError:

        log_callback(
            "ERROR: FFmpeg tidak ditemukan. Pastikan FFmpeg sudah terpasang dan tersedia di PATH."
        )
        ...
        log_callback(
            "Streaming selesai atau dihentikan."
        )
...
def main():
    ...
    st.markdown(
        '<h1>'
        '<a href="https://www.youtube.com/@thexextsolutionid" '
        'target="_blank" '
        'style="text-decoration:none;">'
        'STREMQU MASTERY by TNS'
        '</a>'
        '</h1>',
    ...
    st.markdown(
        '<h4>'
        'Tools live streaming YouTube pribadi tanpa habiskan kuota, '
        'sewa RDP, VPS dll.'
        '</h4>',
        unsafe_allow_html=True
    )
    ...
    # ============================================================
    # DISCLAIMER
    # DEFAULT TERBUKA KARENA MENGGUNAKAN <details open>
    # ============================================================
    ...
    # ============================================================
    # MODE 5 VIDEO PLAYLIST
    # ============================================================

    if mode == "5 Video Playlist":

        st.subheader(
            "Upload Playlist — 5 Video"
        )

        st.caption(
            "Video akan dimainkan sesuai urutan: "
            "Video 1 → Video 2 → Video 3 → Video 4 → Video 5."
        )

        st.info(
            "Setiap slot bisa menggunakan Upload, Link langsung, "
            "atau Google Drive. Video dari Link/Drive diunduh langsung "
            "ke server sehingga tidak perlu upload melalui browser."
        )
        ...
        for slot in range(1, 6):
            ...
            if source == "Upload dari perangkat":
                ...
                if uploaded_file is not None:
                    ...
                    st.session_state[
                        f"playlist_video_path_{slot}"
                    ] = saved

                    st.success(
                        f"Video {slot} siap: {uploaded_file.name}"
                    )
                ...
                video_url = st.text_input(
                    ...
                    help=(
                        "Gunakan direct link yang bisa diakses "
                        "tanpa login dan mengarah ke file video."
                    )
                )
                ...
                if st.button(
                    f"⬇️ Ambil Video {slot} dari Link",
                    key=f"playlist_download_link_{slot}",
                    ...
                    if not video_url.strip():
                        ...
                        st.error(
                            f"Masukkan URL Video {slot} terlebih dahulu."
                        )
                        ...
                        try:
                            ...
                            with st.spinner(
                                f"Mengunduh Video {slot} ke server..."
                            ):

                                saved = download_video_from_url(
                                    video_url,
                                    slot,
                                    link_name
                                )
                            ...
                            st.success(
                                f"Video {slot} siap: {Path(saved).name}"
                            )

                        except Exception as e:

                            st.error(
                                f"Gagal mengambil Video {slot}: {e}"
                            )
                ...
                drive_url = st.text_input(
                    ...
                    help=(
                        "File Google Drive harus dapat diakses "
                        "dengan 'Anyone with the link'."
                    )
                )
                ...
                if st.button(
                    f"☁️ Ambil Video {slot} dari Google Drive",
                    key=f"playlist_download_drive_{slot}",
                    use_container_width=True
                ):
                    ...
                    if not drive_url.strip():
                        ...
                        st.error(
                            f"Masukkan link Google Drive Video {slot} terlebih dahulu."
                        )
                        ...
                        try:
                            ...
                            with st.spinner(
                                f"Mengunduh Video {slot} dari Google Drive ke server..."
                            ):
        ...
        if selected_paths:
            ...
            st.info(
                "Belum ada video. Tambahkan minimal 1 video "
    ...
    # ============================================================
        ...
        st.subheader(
            "Upload Video + MP3 — Playlist 5 MP3"
        )

        st.caption(
            "1 video di-loop terus + MP3 1 → 2 → 3 → 4 → 5. "
            "Mode hemat CPU untuk Streamlit Cloud: "
            "encoding 1080p/20fps, preset ultrafast, "
            "dan bitrate CPU-efisien."
        )

        st.info(
            "Tips: gunakan MP4 H.264 + AAC dan MP3 bitrate normal "
            "(128–320 kbps) agar perpindahan audio lebih lancar."
        )
        ...
        video_source = st.radio(
            ...
            help=(
                "Link/Drive diunduh langsung ke server sehingga "
                "tidak perlu upload ulang lewat browser."
            ),
        )
        ...
        if video_source == "Upload dari perangkat":
            ...
            uploaded_video = st.file_uploader(
                ...
                help=(
                    "Video yang akan di-loop terus "
                    "selama playlist MP3 berjalan."
                ),
            )

            if uploaded_video is not None:

                video_saved = save_uploaded_file(
                    uploaded_video,
                    1
                )

                st.success(
                    f"Video siap: {uploaded_video.name}"
                )
            ...
            video_url = st.text_input(
                ...
                help=(
                    "Gunakan direct link yang bisa diakses "
                    "tanpa login dan mengarah langsung ke file video."
                ),
            )
            ...
            if st.button(
                "⬇️ Ambil Video dari Link",
                key="download_video_link",
                use_container_width=True
            ):
                ...
                if not video_url.strip():
                    ...
                    try:
                        ...
                        with st.spinner(
                            "Mengunduh video ke server..."
                        ):

                            video_saved = download_video_from_url(
                                video_url,
                                1,
                                link_name
                            )
                        ...
                        st.success(
                            f"Video siap: {Path(video_saved).name}"
                        )

                    except Exception as e:

                        st.error(
                            f"Gagal mengambil video: {e}"
                        )

            video_saved = st.session_state.get(
                "remote_video_path"
            )
            ...
            drive_url = st.text_input(
                ...
                key="video_drive_url",
                help=(
                    "File harus bisa diakses dengan "
            ...
            if st.button(
                "☁️ Ambil Video dari Google Drive",
                key="download_video_drive",
                use_container_width=True
            ):
                ...
                if not drive_url.strip():
                    ...
                    st.error(
                        "Masukkan link Google Drive terlebih dahulu."
                    )
                    ...
                    try:
                        ...
                        with st.spinner(
                            "Mengunduh video dari Google Drive ke server..."
                        ):
                        ...
                        st.error(
                            f"Gagal mengambil video dari Google Drive: {e}"
                        )
        ...
        for slot in range(1, 6):
            ...
            uploaded_audio = st.file_uploader(
                ...
                help=(
                    f"MP3 ke-{slot}. Setelah MP3 {slot} selesai, "
                    "lanjut ke MP3 berikutnya."
                ),
            )

            if uploaded_audio is not None:

                audio_saved = save_uploaded_audio(
                    uploaded_audio,
                    slot
                )

                st.success(
                    f"MP3 {slot} siap: {uploaded_audio.name}"
                )
        ...
        if selected_paths:
            ...
            st.write(
                f"**Video:** {Path(selected_paths[0]).name}"
            )
    ...
    playback_mode = st.radio(
        ...
        horizontal=True,
        help=(
            "Tanpa batas = loop terus. "
            "Jumlah pengulangan = jumlah putaran playlist. "
            "Durasi streaming = berhenti otomatis setelah waktu yang dipilih."
        ),
    )
    ...
    if playback_mode == "Jumlah pengulangan":
        ...
        repeat_count = st.number_input(
            ...
            step=1,
            help=(
                "1 = satu kali, 2 = dua kali, dst. "
                "Pada Mode Video + MP3, yang diulang adalah urutan MP3."
            ),
        )
        ...
        duration_hours = st.number_input(
            ...
            step=0.5,
            help=(
                "Streaming akan dihentikan otomatis setelah durasi ini."
            ),
        )
    ...
    with col1:
        ...
        if st.button(
            ...
            if not selected_paths:
                ...
                st.error(
                    "Upload minimal 1 file MP3 terlebih dahulu!"
                )

            elif not stream_key:

                st.error(
                    "Stream Key YouTube harus diisi!"
                )
